...
CREATE OR REPLACE VIEW c_invoice_header_vt AS 
 SELECT i.ad_client_id,
    ...
    oi.taxid,
    dtt.printname AS documenttype,
    dtt.documentnote AS documenttypenote,
    i.c_order_id,
    i.salesrep_id,
    ...
    i.chargeamt,
    i.totallines,
    i.grandtotal + i.withholdingamt AS grandtotal,
    i.grandtotal + i.withholdingamt AS amtinwords,
    i.m_pricelist_id,
    i.istaxincluded,
    i.c_campaign_id,
    i.c_project_id,
    ...
    (case dtfe.diandoctypecode when '01' then 'CUFE: ' when '02' then 'CUFE: ' else 'UUID: ' end || a.lco_fe_dian_uuid)::varchar(128) as cufeuuid,
    a.LCO_FE_IdTransaction as qrdata
    /*
    case when a.lco_printedformcontrol_ID is not null then 
    ('Autorización de facturación No. ' || pfc.AuthorizationNo || ' del ' || to_char(pfc.AuthorizationDate, 'DD-MM-YYYY'::text) || ' de ' || COALESCE(pfc.Prefix, '') || pfc.InitialSequence || ' a ' || COALESCE(pfc.Prefix, '') || pfc.FinalSequence) else NULL end::varchar as note
    */
   FROM c_invoice i
     JOIN c_doctype dt ON i.c_doctype_id = dt.c_doctype_id
     JOIN c_doctype_trl dtt ON i.c_doctype_id = dtt.c_doctype_id
     JOIN c_paymentterm pt ON i.c_paymentterm_id = pt.c_paymentterm_id
     JOIN c_paymentterm_trl ptt ON i.c_paymentterm_id = ptt.c_paymentterm_id AND dtt.ad_language::text = ptt.ad_language::text
     JOIN c_bpartner bp ON i.c_bpartner_id = bp.c_bpartner_id
     LEFT JOIN c_greeting bpg ON bp.c_greeting_id = bpg.c_greeting_id
     LEFT JOIN c_greeting_trl bpgt ON bp.c_greeting_id = bpgt.c_greeting_id AND dtt.ad_language::text = bpgt.ad_language::text
     ...
     LEFT JOIN c_greeting_trl bpcgt ON bpc.c_greeting_id = bpcgt.c_greeting_id AND dtt.ad_language::text = bpcgt.ad_language::text
     ...
     LEFT JOIN c_currency_trl cur ON i.c_currency_id = cur.c_currency_id AND dtt.ad_language::text = cur.ad_language::text
     LEFT JOIN lco_fe_doctype dtfe ON dtfe.lco_fe_doctype_id = dt.lco_fe_doctype_id
     LEFT JOIN lco_fe_authorization a ON a.lco_fe_authorization_id = i.lco_fe_authorization_id
     LEFT JOIN lco_printedformcontrol pfc ON pfc.lco_printedformcontrol_ID = a.lco_printedformcontrol_ID
     -- WHERE i.C_Invoice_ID = 1004609
     -- limit 1
     ;

ALTER TABLE c_invoice_header_vt OWNER TO adempiere;
